...
export async function lygosInitiateMomoPayment(params: { orderId: string; amount: number; currency: string; customerPhone: string; }): Promise<{ paymentUrl?: string; transactionId?: string; provider: string }> {
  ...
  const data = res.data || {};
  return { paymentUrl: data.checkoutUrl || data.paymentUrl, transactionId: data.transactionId || data.id, provider: 'MTN Mobile Money' };
}
