...
export const productController = {
  list: async (req: Request, res: Response) => {
    ...
    const data = await productService.list({ page, limit, q, categoryId });
  ...
  create: async (req: Request, res: Response) => {
    ...
    try { await logActivity({ staffId: (req as any).user?.id, staffRole: (req as any).user?.role, action: 'created_product', entityType: 'product', entityId: (item as any)?._id?.toString?.() || (item as any)?.id, details: `SKU=${payload.sku} name=${payload.name}` }); } catch {}
    res.status(201).json({ success: true, data: item });
  },
  update: async (req: Request, res: Response) => {
    const payload = productSchema.partial().parse(req.body);
    const item = await productService.update(req.params.id, payload);
    try { await logActivity({ staffId: (req as any).user?.id, staffRole: (req as any).user?.role, action: 'updated_product', entityType: 'product', entityId: req.params.id, details: `fields=${Object.keys(payload).join(',')}` }); } catch {}
    res.json({ success: true, data: item });
  },
