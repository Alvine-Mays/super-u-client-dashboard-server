...
type Category = { _id: string; name: string; slug: string };
 type Product = { _id: string; name: string; price: number; images?: string[] };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            {productsData?.total || 0} produit{(productsData?.total || 0) > 1 ? 's' : ''} disponible{(productsData?.total || 0) > 1 ? 's' : ''}
          </p>
        </div>
          ...
          {productsData.total > 20 && (
            <div className="flex items-center justify-center gap-2 mt-12">
              ...
              <span className="text-sm text-muted-foreground px-4">
                Page {page} sur {Math.ceil((productsData.total || 0) / (productsData.limit || 20))}
              </span>
              <Button
                variant="outline"
                disabled={productsData.page * productsData.limit >= productsData.total}
                onClick={() => setPage(p => p + 1)}
